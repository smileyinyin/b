...
export default {
  ...
  methods: {
    getAll() {
      ...
      orderList({parent_id: this.$store.state.user.userId, order_uuid: this.order_uuid, order_status: this.order_status, create_time: starttime, end_time: endtime, pageNum: this.currentPage, pageSize: this.pageSize, is_since: 1, parent_role_id: this.$store.state.user.roleId}).then(res => {
        that.data = res.data
