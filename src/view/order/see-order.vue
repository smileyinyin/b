<template>
     <div>
      ...
      {title: '查看订单详情'}
          ...
          <div  v-if="params.status === 1" style="100%">
                    <Button type="primary"  style="width:120px;float:right;margin-top:30px;margin-left:30px" @click="modal1=true">确认核销</Button>
                    <Button type="primary"  style="width:120px;float:right;margin-top:30px;margin-right:30px" @click="modal2=true">取消订单</Button>
                </div>
     </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    ok1() {
      this.order_uuid = this.$route.query.order_uuid
      orderhexiao({order_uuid: this.order_uuid, account_code: 123454, order_note: this.order_note}).then(res => {
      })
    },
    ok2() {
      this.order_uuid = this.$route.query.order_uuid
      ordersntuikuan({order_uuid: this.order_uuid, type: 0}).then(res => {

      })
    },
    ok3() {
      ordersntuikuan({id: this.order_uuid, type: 1}).then(res => {
